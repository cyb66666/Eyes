...
from model.tools.multi_scale_block import MultiScaleBlock
import model.tools.classifier_block as classifier
import model.tools.preresnet_dnn_block as preresnet_dnn
...
if __name__ == '__main__':
    ...
    model = coattnet_v2_withWeighted_tiny(num_classes=8).to(device)
    inputs = torch.rand([16,3,512,1024]).to(device)
    print(model(inputs))
