...
from  model.tools.common import DropPath, Mlp
...
class MultiScaleAttention(nn.Module):
    def __init__(
        ...
        if pool_first:
            ...
            # self.to_q = nn.Conv2d(dim, dim * 1, 1, bias=qkv_bias)
            # self.to_kv = nn.Conv2d(dim, dim * 2, 1, bias=qkv_bias)
...
class MultiScaleBlock(nn.Module):
    ...
    def forward(self, x, hw_shape):
        x = rearrange(x, 'b d x y -> b x y d', y=x.shape[3])
